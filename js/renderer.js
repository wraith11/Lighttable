...
export class GameRenderer {
    ...
    updateFoWMemory(forceRebuild = false) {
        ...
        for (let i = startIdx; i < visited.length; i++) {
            ...
            if (poly.length > 0) {
                // Inset: Polygon-Punkte um 2px zum Zentrum (pt) hin verschieben, damit die
                // weiße Fläche nicht exakt bis zur Wandkante reicht. Verhindert harte
                // senkrechte Linien an Wandkanten nach dem Blur (permanenter FoW).
                const inset = 2.0 * ms;
        ...
        Object.values(this.scene.tokens).forEach(t => {
            if (t.has_vision) { 
                ...
                if (t._visionKey !== vKey) {
                    t._visionPoly = calculateVisibility({x:t.x, y:t.y, radius: t.vision_range}, segments, this._visGridCache);
                    t._visionKey = vKey;
                }
